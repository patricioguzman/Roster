...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt Optimization: Resolve access control first before triggering parallel fetches
        ...
        // ⚡ Bolt Optimization: Construct parallel promises to avoid sequential execution
        const settingsPromise = db.query('SELECT * FROM settings');

        // ⚡ Bolt Optimization: Push array filters into SQL WHERE clauses instead of in-memory filtering
        let storesPromise;
        if (allowedStoreIds !== null) {
            if (allowedStoreIds.length === 0) {
                ...
                storesPromise = db.query(`SELECT * FROM stores WHERE id IN (${placeholders})`, allowedStoreIds);
            }
        } else {
            storesPromise = db.query('SELECT * FROM stores');
        }
        ...
        const membersQuery = `
            ...
            GROUP BY m.id
        `;
        const membersPromise = db.query(membersQuery);

        const mgrStoresPromise = (async () => {
            try { return await db.query('SELECT member_id, store_id FROM manager_stores'); } catch (e) { return []; }
        })();
        ...
        if (allowedStoreIds !== null) {
            if (allowedStoreIds.length === 0) {
                ...
                const placeholders = allowedStoreIds.map(() => '?').join(',');
                shiftsPromise = db.query(`SELECT * FROM shifts WHERE store_id IN (${placeholders})`, allowedStoreIds);
            }
        } else {
            shiftsPromise = db.query('SELECT * FROM shifts');
        }

        let currentUserManagedStoreIdsPromise = Promise.resolve([]);
        if (user && user.role === 'manager') {
            currentUserManagedStoreIdsPromise = db.query('SELECT store_id FROM manager_stores WHERE member_id = ?', [user.id]);
        }

        // ⚡ Bolt Optimization: Execute independent database queries concurrently
        const [settingsRows, stores, members, mgrStores, shifts, currentUserManagedStoreRows] = await Promise.all([
            settingsPromise,
            storesPromise,
            membersPromise,
            mgrStoresPromise,
            shiftsPromise,
            currentUserManagedStoreIdsPromise
        ]);
