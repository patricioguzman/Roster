...
app.post('/api/worked-hours', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await db.transaction(async (tx) => {
            ...
            const validData = hoursData.filter(h => h.ordinary || h.sat || h.sun || h.ph || h.al || h.sl || h.notes);
            if (validData.length === 0) return;

            const uploadedAt = new Date().toISOString();

            // Chunked batch insert for optimal cross-database compatibility (SQLite/MySQL)
            const chunkSize = 50;
            for (let i = 0; i < validData.length; i += chunkSize) {
                const chunk = validData.slice(i, i + chunkSize);
                const placeholders = chunk.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').join(', ');
                const params = chunk.flatMap(h => [
                    storeId, h.memberId, date,
                    h.ordinary || 0, h.sat || 0, h.sun || 0, h.ph || 0, h.al || 0, h.sl || 0,
                    h.notes || '', req.user.id, uploadedAt
                ]);
