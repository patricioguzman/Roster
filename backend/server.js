    ...
    try {
        ...
        if (user && user.role === 'employee') {
            ...
            const [ms1, ms2] = await Promise.all([
                ...
                (async () => {
                    try { return await db.query('SELECT store_id FROM manager_stores WHERE member_id = ?', [user.id]); }
                    catch (e) { return []; }
                })()
            ]);
        ...
        // ⚡ Bolt Performance: Build promises to fetch data concurrently
        ...
        if (allowedStoreIds !== null && allowedStoreIds.length === 0) {
            ...
            storesPromise = db.query(storesQuery, storesParams);
        }
        ...
        if (allowedStoreIds !== null && allowedStoreIds.length === 0) {
            ...
            shiftsPromise = db.query(shiftsQuery, shiftsParams);
        }

        let currentUserManagedStoreIdsPromise = Promise.resolve([]);
        if (user && user.role === 'manager') {
            currentUserManagedStoreIdsPromise = (async () => {
                try { return await db.query('SELECT store_id FROM manager_stores WHERE member_id = ?', [user.id]); }
                catch (e) { return []; }
            })();
        }

        // ⚡ Bolt Performance: Parallelize database queries and map results
        const [
            settingsRows,
            stores,
            members,
        ...
        if (user) {
            ...
            if (user.role === 'manager') {
                data.currentUserManagedStoreIds = currentUserManagedStoreIds.map(r => r.store_id);
            }
        }
...
if (require.main === module) { app.listen(PORT, () => { console.log(`Roster Server running on http://localhost:${PORT}`); }); } else { module.exports = { app, authenticateToken, requireAdmin }; }
