...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt: Resolve store access sequentially first because subsequent queries rely on allowedStoreIds
        ...
        if (user && user.role === 'employee') {
            ...
            // ⚡ Bolt: Parallelize store access queries for manager
        ...
        // ⚡ Bolt: Determine if we need to filter stores and shifts in SQL
        const hasStoreFilter = allowedStoreIds !== null;
        ...
        if (hasStoreFilter) {
            if (allowedStoreIds.length === 0) {
                // If they have no access, return empty array immediately without DB call
                storesQuery = null;
                shiftsQuery = null;
            } else {
        ...
        // ⚡ Bolt: Parallelize data fetching using Promise.all to reduce network latency
        const [settingsRows, stores, members, mgrStores, shifts, userManagedStores] = await Promise.all([
            db.query('SELECT * FROM settings'),
            storesQuery ? db.query(storesQuery, storesParams) : Promise.resolve([]),
            db.query(membersQuery),
            (async () => { try { return await db.query('SELECT member_id, store_id FROM manager_stores'); } catch (e) { return []; } })(),
            shiftsQuery ? db.query(shiftsQuery, shiftsParams) : Promise.resolve([]),
            (user && user.role === 'manager') ? db.query('SELECT store_id FROM manager_stores WHERE member_id = ?', [user.id]) : Promise.resolve([])
        ]);
        ...
        if (user) {
            ...
            if (user.role === 'manager') {
                data.currentUserManagedStoreIds = userManagedStores.map(r => r.store_id);
            }
        }
