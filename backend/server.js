...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt Optimization: Parallelize independent DB queries and push filter logic into SQL to reduce overhead
        const [settingsRows, stores, members, mgrStores, shifts] = await Promise.all([
            db.query('SELECT * FROM settings'),
            shouldFetchData ? db.query(storesQuery, storesParams) : Promise.resolve([]),
            db.query(membersQuery),
            (async () => { try { return await db.query('SELECT member_id, store_id FROM manager_stores'); } catch (e) { return []; } })(),
            shouldFetchData ? db.query(shiftsQuery, shiftsParams) : Promise.resolve([])
        ]);
