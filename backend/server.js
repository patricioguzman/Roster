...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt: Push allowedStoreIds filter to SQL to reduce memory usage and data transfer
        ...
        // ⚡ Bolt: Parallelize independent data fetching
        const [
            settingsRows,
            stores,
            members,
            ...
            (async () => {
                if (user && user.role === 'manager') {
                    try { return await db.query('SELECT store_id FROM manager_stores WHERE member_id = ?', [user.id]); } catch (e) { return []; }
                }
                return [];
            })()
        ]);
        ...
        data.members = members.map(m => {
            const memberStoreIds = m.store_ids ? String(m.store_ids).split(',').map(id => parseInt(id, 10)) : [];
