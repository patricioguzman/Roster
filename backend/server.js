...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt: Parallelized independent database queries using Promise.all to reduce cumulative network latency
        const [settingsRows, storesRaw, members, mgrStoresRaw, shiftsRaw, managerStoresRaw] = await Promise.all([
        ...
        let shifts = shiftsRaw;
        if (allowedStoreIds !== null) {
            shifts = shiftsRaw.filter(s => allowedStoreIds.includes(s.store_id));
        }
        ...
        if (user) {
            ...
            if (user.role === 'manager') {
                data.currentUserManagedStoreIds = managerStoresRaw.map(r => r.store_id);
            }
        }
