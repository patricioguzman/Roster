    ...
    try {
        await db.transaction(async (tx) => {
            ...
            if (saveShifts && saveShifts.length > 0) {
                // ⚡ Bolt: Use in-memory dictionary caching for repeated lookup queries during batch processing
                const memberCache = {};
                for (let s of saveShifts) {
                    if (s.id && typeof s.id === 'string' && s.id.startsWith('new_')) {
                        const cacheKey = `${s.storeId}_${s.name}`;
                        if (!memberCache[cacheKey]) {
                            ...
                            if (row) memberCache[cacheKey] = row.id;
                        }
