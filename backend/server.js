...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt Optimization: Parallelize independent DB queries with Promise.all
        // and push down allowedStoreIds filtering to the SQL query to minimize data transfer and latency.
        let storeQuery = 'SELECT * FROM stores';
        let storeParams = [];
        let shiftQuery = 'SELECT * FROM shifts';
        let shiftParams = [];

        // Push filter down to DB if limited access
        if (allowedStoreIds !== null) {
            if (allowedStoreIds.length === 0) {
                // Short circuit if no stores allowed
                storeQuery = null;
                shiftQuery = null;
            } else {
                const placeholders = allowedStoreIds.map(() => '?').join(',');
                storeQuery += ` WHERE id IN (${placeholders})`;
                storeParams = allowedStoreIds;
                shiftQuery += ` WHERE store_id IN (${placeholders})`;
                shiftParams = allowedStoreIds;
            }
        }
