...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt: Push array filters into SQL WHERE clauses to reduce data transfer and memory overhead
        let storesPromise;
        let shiftsPromise;
        ...
        if (user && user.role === 'manager') {
            currentUserManagedStoreIdsPromise = db.query('SELECT store_id FROM manager_stores WHERE member_id = ?', [user.id])
                .then(ms => ms.map(r => r.store_id))
                .catch(() => []);
        }

        // ⚡ Bolt: Execute independent queries in parallel using Promise.all to minimize latency
        const [
            settingsRows,
            stores,
            members,
