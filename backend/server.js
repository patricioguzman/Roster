...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // Bolt optimization: Fetch independent queries in parallel using Promise.all
        const [
            settingsRows,
            stores,
            members,
        ...
        if (user) {
            ...
            if (user.role === 'manager') {
                data.currentUserManagedStoreIds = userManagedStoreIdsMs.map(r => r.store_id);
            }
        }
