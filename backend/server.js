    ...
    try {
        await db.transaction(async (tx) => {
            ...
            if (saveShifts && saveShifts.length > 0) {
                ...
                // Caching member lookups to prevent N+1 query problem
                // Batching new inserts into a single parameter array to dramatically reduce DB roundtrip latency
                const memberCache = {};
                const newShiftsArgs = [];
                const updateShiftsArgs = [];

                for (let s of saveShifts) {
                    if (typeof s.id === 'number') {
                        updateShiftsArgs.push([s.startTime, s.endTime, s.duration, s.id]);
                    } else {
                        ...
                        if (memberId) {
                            newShiftsArgs.push([s.storeId, memberId, s.name, s.date, s.startTime, s.endTime, s.duration]);
                        }
                    }
                }

                // ⚡ Bolt: Execute batched insert
                if (newShiftsArgs.length > 0) {
                    const placeholders = newShiftsArgs.map(() => '(?, ?, ?, ?, ?, ?, ?)').join(', ');
                    const params = newShiftsArgs.flat();
                ...
                for (let args of updateShiftsArgs) {
                    await tx.run(`UPDATE shifts SET start_time = ?, end_time = ?, duration = ? WHERE id = ?`, args);
                }
            }
        });
        res.json({ success: true });
