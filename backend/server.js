...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt: Parallelize DB reads and push filters to SQL WHERE clauses
        ...
        const mgrStoresPromise = (async () => {
            try { return await db.query('SELECT member_id, store_id FROM manager_stores'); }
            catch { return []; }
        })();
        ...
        let userMgrStoresPromise = Promise.resolve([]);
        if (user && user.role === 'manager') {
            userMgrStoresPromise = db.query('SELECT store_id FROM manager_stores WHERE member_id = ?', [user.id]);
        }

        const [settingsRows, stores, members, mgrStores, shifts, userMgrStores] = await Promise.all([
            settingsPromise, storesPromise, membersPromise, mgrStoresPromise, shiftsPromise, userMgrStoresPromise
        ]);
        ...
        if (user) {
            ...
            if (user.role === 'manager') {
                data.currentUserManagedStoreIds = userMgrStores.map(r => r.store_id);
            }
        }
