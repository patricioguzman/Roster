...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // Bolt: Push store filtering into SQL to reduce memory and data transfer
        ...
        // Bolt: Parallelize independent DB queries for faster load
        const [
            settingsRows,
            stores,
            members,
        ...
        if (user) {
            ...
            if (user.role === 'manager') {
                data.currentUserManagedStoreIds = currentUserManagerStores.map(r => r.store_id);
            }
        }
