...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // ⚡ Bolt Optimization: Parallelize independent database reads to reduce network round-trip time.
        const [settingsRows, storesUnfiltered, membersUnfiltered, mgrStoresFetched, shiftsUnfiltered] = await Promise.all([
        ...
        data.members = membersUnfiltered.map(m => {
            const memberStoreIds = m.store_ids ? String(m.store_ids).split(',').map(id => parseInt(id)) : [];
            const managedStoreIds = mgrStoresFetched.filter(row => row.member_id === m.id).map(row => row.store_id);
            return {
