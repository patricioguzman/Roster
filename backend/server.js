...
app.get('/api/data', async (req, res) => {
    try {
        ...
        // Ensure dependent variables like allowedStoreIds are resolved sequentially first
        let storesQuery = 'SELECT * FROM stores';
        let shiftsQuery = 'SELECT * FROM shifts';
        let queryParams = [];

        // Build parameterized queries for stores and shifts if filtering by allowedStoreIds
        if (allowedStoreIds !== null) {
            if (allowedStoreIds.length === 0) {
                // If allowedStoreIds is empty, we still need valid syntax that returns 0 rows
                storesQuery += ' WHERE id IN (NULL)';
                shiftsQuery += ' WHERE store_id IN (NULL)';
            } else {
