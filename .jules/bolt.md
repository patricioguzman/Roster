...
## 2024-05-19 - Concurrent Database Queries and Filtering in /api/data
**Learning:** The `/api/data` endpoint performs 5 sequential database queries (`settings`, `stores`, `members`, `manager_stores`, `shifts`) and then performs in-memory filtering of `stores` and `shifts` based on `allowedStoreIds`. This creates significant cumulative network latency and overhead.
**Action:** Always wrap independent database queries in `Promise.all` instead of `await`ing sequentially. Additionally, push filtering (like `allowedStoreIds`) down to the database using `WHERE ... IN` instead of fetching all rows and filtering in memory to minimize data transfer and processing.
